...
def load_images():
    ...
    Button(janela, text="<<", command= back, state=DISABLED).grid(row=0, column=0)
    Button(janela, text=">>", command= forward(2), state=DISABLED).grid(row=0, column=3)
    Button(janela, text="Selecionar pasta", command=lambda: load_images()).grid(row=1, column=1, columnspan=2)
